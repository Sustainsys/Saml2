...
partial class SamlXmlReaderTests
{
    ...
    [Arguments("./samlp:Status", ErrorReason.MissingElement)]
    [Arguments("./samlp:Status/samlp:StatusCode", ErrorReason.MissingElement)]
    [Arguments("./samlp:Status/samlp:StatusCode/@Value", ErrorReason.MissingAttribute)]
    public void ReadResponse_MissingMandatory(string removeXPath, ErrorReason expectedError)
    {
        ...
        var subject = new SamlXmlReader();
        subject.Invoking(s => s.ReadResponse(source)).Should().Throw<SamlXmlException>().WithErrors(expectedError);
    }
    ...
    public void ReadResponse_SignedWithoutIssuerFails()
    {
        ...
        var subject = new SamlXmlReader();
        subject.Invoking(s => s.ReadResponse(source)).Should().Throw<SamlXmlException>().WithErrors(ErrorReason.MissingElement);
    }
