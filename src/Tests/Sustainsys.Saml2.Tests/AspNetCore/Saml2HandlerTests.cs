...
public class Saml2HandlerTests
{
    ...
    private static async Task<(Saml2Handler subject, HttpContext httpContext)> CreateSubject(Saml2Options options)
    {
        ...
        keyedServiceProvider.GetRequiredKeyedService(typeof(IEnumerable<IFrontChannelBinding>), Arg.Any<string>()).Returns(Enumerable.Empty<IFrontChannelBinding>());
        keyedServiceProvider.GetService(typeof(IEnumerable<IFrontChannelBinding>)).Returns(new IFrontChannelBinding[] { new HttpRedirectBinding(), new HttpPostBinding() });
        var handler = new Saml2Handler(optionsMonitor, loggerFactory, UrlEncoder.Default, keyedServiceProvider);
        FakeTimeProvider timeProvider = new FakeTimeProvider(new(2025, 05, 28, 11, 14, 53, TimeSpan.Zero));
        keyedServiceProvider.GetService(typeof(IValidator<Response, ResponseValidationParameters>)).Returns(new ResponseValidator(new AssertionValidator(timeProvider)));
        ...
        httpContext.RequestServices.GetService(Arg.Is<Type>(t => t == typeof(IAuthenticationService))).Returns(authenticationService);
    ...
    private XmlDocument GetSignedXmlDoc(string xPath, [CallerMemberName] string? name = null)
    {
        ...
        var signedElement = (XmlElement? )xmlDoc.SelectSingleNode(xPath, xmlDoc.GetNsMgr()) ?? throw new ArgumentException();
    ...
    public async Task ChallengeAsync()
    {
        ...
        cookieManager.AppendResponseCookie(httpContext, Arg.Do<string>(validateCookieName), Arg.Do<string>(validateCookieContents), Arg.Do<CookieOptions>(validateCookieOptions));
        ...
        void validateLocation(string location)
        {
            ...
            var deserializedAuthnRequest = new SamlXmlReader().ReadAuthnRequest(message.Xml.GetXmlTraverser());
        ...
        cookieManager.Received().AppendResponseCookie(Arg.Any<HttpContext>(), Arg.Any<string>(), Arg.Any<string?>(), Arg.Any<CookieOptions>());
    }
    ...
    public async Task HandleRemoteAuthenticate_CannotUnbind()
    {
        ...
        await subject.Invoking(async s => await s.HandleRequestAsync()).Should().ThrowAsync<AuthenticationFailureException>().WithInnerException<AuthenticationFailureException, AuthenticationFailureException>().WithMessage("No binding*");
    }

    [Test]
    [Arguments("/samlp:Response/saml:Assertion")]
    [Arguments("/samlp:Response")]
    ...
    public async Task HandleRemoteAuthenticate(string xPathToSign)
    {
        ...
        httpContext.Request.Form = new FormCollection(new() { { "SAMLResponse", new(encodedResponse) }, { "RelayState", relayState } });
        var authProps = new AuthenticationProperties();
        authProps.Items[".idp"] = "https://idp.example.com";
        authProps.Items[".reqId"] = "abc123";
        authProps.Items["TestKey"] = "TestValue";
        options.StateCookieManager.GetRequestCookie(httpContext, cookieName).Returns(options.StateCookieDataFormat.Protect(authProps));
        ...
        options.StateCookieManager.Received().DeleteCookie(httpContext, cookieName, Arg.Any<CookieOptions>());
        await authenticationService.Received().SignInAsync(Arg.Any<HttpContext>(), Arg.Is<string?>(value: null), Arg.Is<ClaimsPrincipal>(p => p.FindFirstValue("email") == "Someone@example.com"), Arg.Is<AuthenticationProperties>(p => p.Items["TestKey"] == "TestValue"));
    }
    ...
    public async Task HandleRemoteAuthenticate_RejectsRequestIdDoesntMatchState()
    {
        ...
        options.StateCookieManager.GetRequestCookie(httpContext, cookieName).Returns(options.StateCookieDataFormat.Protect(authProps));
        await subject.Invoking(s => s.HandleRequestAsync()).Should().ThrowAsync<AuthenticationFailureException>().WithInnerException<AuthenticationFailureException, ValidationException<Saml2Message>>().WithMessage("*InResponseTo*");
    }
    ...
    public async Task HandleRemoteAuthenticate_RejectsRequestIdDoesntMatchInResponseTo()
    {
        ...
        authProps.Items[".reqId"] = "456";
        options.StateCookieManager.GetRequestCookie(httpContext, cookieName).Returns(options.StateCookieDataFormat.Protect(authProps));
        await subject.Invoking(s => s.HandleRequestAsync()).Should().ThrowAsync<AuthenticationFailureException>().WithInnerException<AuthenticationFailureException, ValidationException<Response>>().WithMessage("*InResponseTo*");
    }
    ...
    public async Task HandleRemoteAuthenticate_RejectsStoredIssuerDoesntMatchState()
    {
        ...
        var xmlDoc = TestData.GetXmlDocument<Saml2HandlerTests>();
        var encodedResponse = Convert.ToBase64String(Encoding.UTF8.GetBytes(xmlDoc.OuterXml));
        httpContext.Request.Form = new FormCollection(new() { { "SAMLResponse", new(encodedResponse) }, { "RelayState", "abc.123" } });
        var authProps = new AuthenticationProperties();
        authProps.Items[".idp"] = "https://idp.example.com";
        options.StateCookieManager.GetRequestCookie(httpContext, "Saml2.abc").Returns(options.StateCookieDataFormat.Protect(authProps));
        await subject.Invoking(s => s.HandleRequestAsync()).Should().ThrowAsync<AuthenticationFailureException>().WithInnerException<AuthenticationFailureException, ValidationException<Saml2Message>>().WithMessage("*RelayState*Idp*");
    }
    ...
    public async Task HandleRemoteAuthenticate_RejectsOnMissingStateCookie()
    {
        ...
        var encodedResponse = Convert.ToBase64String(Encoding.UTF8.GetBytes(xmlDoc.OuterXml));
        httpContext.Request.Form = new FormCollection(new() { { "SAMLResponse", new(encodedResponse) }, { "RelayState", "abc.123" } });
        await subject.Invoking(s => s.HandleRequestAsync()).Should().ThrowAsync<AuthenticationFailureException>().WithInnerException<AuthenticationFailureException, ValidationException<Saml2Message>>().WithMessage("*cookie*");
    }
    ...
    public async Task HandleRemoteAuthenticate_RejectsOnMissingRelaystateParam()
    {
        ...
        var xmlDoc = TestData.GetXmlDocument<Saml2HandlerTests>();
        var encodedResponse = Convert.ToBase64String(Encoding.UTF8.GetBytes(xmlDoc.OuterXml));
        httpContext.Request.Form = new FormCollection(new() { { "SAMLResponse", new(encodedResponse) }, });
        await subject.Invoking(s => s.HandleRequestAsync()).Should().ThrowAsync<AuthenticationFailureException>().WithInnerException<AuthenticationFailureException, ValidationException<Saml2Message>>().WithMessage("*RelayState*");
    }
