...
public class ClaimsFactoryTests
{
    ...
    public void GetClaimsIdentity()
    {
        ...
        Claim[] expectedClaims = [new(ClaimTypes.NameIdentifier, "NameIdValue"), new("email", "john.doe@example.com"), new("role", "admin"), new("role", "boss")];
        ClaimsIdentity expected = new ClaimsIdentity(expectedClaims, "saml2");
        actual.Should().BeEquivalentTo(expected, opt => opt.For(ci => ci.Claims).Exclude(c => c.Subject));
    }
}