...
{
    public static readonly string[] allowedHashes =
    {
        "sha256"
    };
    ...
    public void Sign()
    {
        ...
        signature["SignedInfo", SignedXml.XmlDsigNamespaceUrl]!["Reference", SignedXml.XmlDsigNamespaceUrl]!.Attributes["URI"]!.Value.Should().Be("#id123");
        ...
        signature["SignedInfo"]!["SignatureMethod"]!.GetAttribute("Algorithm").Should().Be(SignedXml.XmlDsigRSASHA256Url);
        signature["SignedInfo"]!["Reference"]!["DigestMethod"]!.GetAttribute("Algorithm").Should().Be(SignedXml.XmlDsigSHA256Url);
    ...
    public void Sign_SignatureLocation_Null()
    {
        ...
        var xmlDoc = new XmlDocument();
        xmlDoc.LoadXml(xml);
        xmlDoc.DocumentElement!.Invoking(de => de.Sign(TestData.Certificate, null !)).Should().Throw<ArgumentNullException>().WithParameterName("insertAfter");
    }
    ...
    public void Sign_SignatureLocation_ImmediateChild()
    {
        ...
        xmlDoc.DocumentElement!.Invoking(de => de.Sign(TestData.Certificate, bNode)).Should().Throw<ArgumentException>();
    }
    ...
    {
        var xml = "<xml ID=\"someID\"><content ID=\"content1\">text</content>" + "<injected>other text</injected></xml>";
        var xmlDoc = new XmlDocument();
        xmlDoc.LoadXml(xml);
        ...
        var(error, workingKey) = injected["Signature", SignedXml.XmlDsigNamespaceUrl]!.VerifySignature(TestData.SingleSigningKey, allowedHashes);
    ...
    public void VerifySignature_NoReference()
    {
        ...
        var signature = xmlDoc.DocumentElement!["Signature", SignedXml.XmlDsigNamespaceUrl]!;
        signature.VerifySignature(TestData.SingleSigningKey, allowedHashes).Error.Should().Contain("reference");
    }
    ...
    public void VerifySignature_EmptyReferenceUri()
    {
        ...
        xd.DocumentElement["Signature"]!.VerifySignature(TestData.SingleSigningKey, allowedHashes).Error.Should().Match("Empty reference*");
    }
    ...
    public void VerifySignature_MultipleReferences()
    {
        ...
        signedXml.ComputeSignature();
        xd.DocumentElement!.AppendChild(signedXml.GetXml());
        xd.DocumentElement["Signature"]!.VerifySignature(TestData.SingleSigningKey, allowedHashes).Error.Should().Match("*Signature*one reference*");
    }
    ...
    public void VerifySignature_IncorrectTransforms()
    {
        ...
        signedXml.ComputeSignature();
        xd.DocumentElement!.AppendChild(signedXml.GetXml());
        xd.DocumentElement["Signature"]!.VerifySignature(TestData.SingleSigningKey, allowedHashes).Error.Should().Match("Transform*");
    }
    ...
    public void VerifySignature_MultipleSignaturesInSameDocument()
    {
        ...
        elemA["Signature"]!.VerifySignature(TestData.SingleSigningKey, allowedHashes).Error.Should().BeNull("First signature should validate");
        elemB["Signature"]!.VerifySignature(TestData.SingleSigningKey, allowedHashes).Error.Should().BeNull("Second signature should validate");
    }
    ...
    public void VerifySignature_SigningAlgorithmStrength()
    {
        ...
        signedXml.ComputeSignature();
        xd.DocumentElement!.AppendChild(signedXml.GetXml());
        xd.DocumentElement["Signature"]!.VerifySignature(TestData.SingleSigningKey, allowedHashes).Error.Should().Match("Signature*algorithm*sha1*");
    }
    ...
    public void VerifySignature_DigestAlgorithmStrength()
    {
        ...
        signedXml.ComputeSignature();
        xd.DocumentElement!.AppendChild(signedXml.GetXml());
        xd.DocumentElement["Signature"]!.VerifySignature(TestData.SingleSigningKey, allowedHashes).Error.Should().Match("Digest*algorithm*sha1*");
    }
    ...
    public void IsSignedByAny()
    {
        ...
        xd.DocumentElement!["Signature"]!.VerifySignature(keys, allowedHashes).Error.Should().BeNull();
    }
    ...
    public void IsSignedByAny_NoKeyMatches_ButEmbeddedKeyValidatesContent()
    {
        ...
        xd.DocumentElement!["Signature"]!.VerifySignature(TestData.SingleSigningKey2, allowedHashes).Error.Should().Match("*validated*contained*trusted*");
    }
    ...
    public void VerifySignature_RejectsDuplicateIdsEvenIfCaseDiffer()
    {
        ...
        b["Signature"]!.Invoking(s => s.VerifySignature(TestData.SingleSigningKey, allowedHashes)).Should().Throw<CryptographicException>().WithMessage("Reference*one*");
    }
    ...
    public void VerifySignature_RejectXmlLowerCaseIDs()
    {
        ...
        signedXml.ComputeSignature();
        xd.DocumentElement!.AppendChild(signedXml.GetXml());
        xd.DocumentElement!["Signature"]!.Invoking(s => s.VerifySignature(TestData.SingleSigningKey, allowedHashes)).Should().Throw<CryptographicException>().WithMessage("Reference*ID*uppercase*");
    }

    private class SignRootSignedXml : System.Security.Cryptography.Xml.SignedXml
    {
        ...
        public override XmlElement GetIdElement(XmlDocument? document, string idValue) => document!.DocumentElement!;
    }
    ...
    public void VerifySignature_RequiresReferenceNCName()
    {
        ...
        signedXml.ComputeSignature();
        xd.DocumentElement!.AppendChild(signedXml.GetXml());
        xd.DocumentElement!["Signature"]!.Invoking(s => s.VerifySignature(TestData.SingleSigningKey, allowedHashes)).Should().Throw<XmlException>();
    }
}