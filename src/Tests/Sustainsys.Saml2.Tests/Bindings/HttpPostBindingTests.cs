...
public class HttpPostBindingTests
{
    ...
    public void CanUnbind_RejectsGet()
    {
        ...
        request.Form = new FormCollection(new() { { "SAMLRequest", "foo" } });
        var subject = new HttpPostBinding();
        subject.CanUnbind(request).Should().BeFalse();
    }

    [Test]
    [Arguments("SAMLRequest", true)]
    [Arguments("SAMLResponse", true)]
    [Arguments("id_token", false)] // Naahh... that would be OIDC
    public void CanUnbind_Accepts(string formKey, bool expected)
    {
        var request = Substitute.For<HttpRequest>();
        request.Method = "POST";
        request.Form = new FormCollection(new() { { formKey, "foo" } });
        var subject = new HttpPostBinding();
        subject.CanUnbind(request).Should().Be(expected);
    }

    [Test]
    [Arguments("SAMLRequest")]
    [Arguments("SAMLResponse")]
    public async Task Unbind(string messageName)
    {
        ...
        request.Method = "POST";
        request.Form = new FormCollection(new() { { messageName, "PHhtbD48YS8+PC94bWw+" }, { "RelayState", "ABC123" } });
        var subject = new HttpPostBinding();
        Func<string, Task<Saml2Entity>> getEntity = str => Task.FromResult<Saml2Entity>(new IdentityProvider());
    ...
    public async Task Unbind_BothResponseAndRequest()
    {
        ...
        request.Path = "/Saml2/Acs";
        request.Method = "POST";
        request.Form = new FormCollection(new() { { "SAMLResponse", "PHhtbD48Yi8+PC94bWw+" }, { "SAMLRequest", "PHhtbD48YS8+PC94bWw+" }, { "RelayState", "ABC123" } });
        var subject = new HttpPostBinding();
        Func<string, Task<Saml2Entity>> getEntity = str => Task.FromResult<Saml2Entity>(new IdentityProvider());
        await subject.Invoking(s => s.UnbindAsync(request, getEntity)).Should().ThrowAsync<ArgumentException>().WithMessage("*both*");
    }
}